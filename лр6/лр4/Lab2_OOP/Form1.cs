...
namespace Lab4_OOP
{
    public partial class Form1 : Form
    {
        ...
        {
            if (!(item == 'M' || item == 'E' || item == 'D' || item == 'I' ||item == ';' || item == '[' ||
             item == ']'))
            {
                return true;
            }
            ...
            try
            {
                ...
                else if (op.symbolOperator == 'I')
                {
                    if (operands.Count == 0)
                    {
                        ...
                        {
                            figure = f as Ellipse;
                            resultInfo += $"Имя: {figure.name}. Координаты: ({figure.x}; {figure.y})." 
                            + $"Ширина: {figure.width}. Высота: {figure.height}\n";
                        }
                        if (resultInfo != "")
                        {
                            MessageBox.Show(resultInfo, "Информация об эллипсах");
                            log.Items.Add($"Информация о фигурах получена");
                        }
                        else
                        {
                            MessageBox.Show($"Фигур нет");
                            log.Items.Add($"Невозможно получить информацию о фигурах");
                        }
                    }
                    else
                    {
                        MessageBox.Show("Опертор I принимает 0 параматетров.");
                        log.Items.Add("Неверное число параметров для оператора D.");
                    }
                }
            }
